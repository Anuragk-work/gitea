=== FILE: docs/16-cli-admin/README.md ===
=== METADATA: format=md ===
[SUMMARY] # CLI & Admin Operations Reference documentation for Gitea's command-line administration tooling. Gitea ships as a single static binary that acts both as the web application and as the command-line administration tool, built on `urfave/cli` in `cmd/`. Every operational task — starting the web server, running database migrations, managing users, backing up data, regenerating SSH keys, or invoking internal Git hooks — is implemented as a CLI subcommand.
