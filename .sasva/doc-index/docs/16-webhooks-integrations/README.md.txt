=== FILE: docs/16-webhooks-integrations/README.md ===
=== METADATA: format=md ===
[SUMMARY] # Webhooks & Integrations Covers Gitea's outgoing webhook system and its integration surfaces with external services and third-party tools. Outbound webhooks are Gitea's primary "push" integration mechanism — repository/organization/ system-level event delivery to external URLs, with built-in adapters for Slack, Discord, Dingtalk, Telegram, MSTeams, Feishu, Matrix, WeChat Work, and Packagist.
